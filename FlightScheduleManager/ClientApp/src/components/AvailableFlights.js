...
/*
 * This component gets the flights from the master list
 * that the user isn't already assigned to AND
 * that do not have a full contingent of attendants
 */

export class AvailableFlights extends Component {
  static displayName = AvailableFlights.name;
  ...
  async loadFlights() {
    ...
    let response = await fetch('api/flights?queryType=available', {
      headers: {
  ...
  onRenderCell(item, index) {
    return(
      <FlightTile tileWidth={`${100 / this.columnCount}%`} flight={item} selectable={true} onSelected={this.onFlightSelected.bind(this, item)} />
    );
  }
